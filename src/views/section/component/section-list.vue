...
const getSectionNews = () => {
  ...
  newsApi
    .sectionNewsList(query.value)
    .then((data) => {
      sectionNewsList.value = data.pageData
...
const sectionId = computed(() => props.sectionId) // Corrected computed property
...
<template>
  <el-scrollbar height="630px">
    <el-empty
      v-if="sectionNewsList.length === 0"
      :image-size="320"
      :image="config.emptyImg"
      description="该栏目下暂未发布新闻"
    ></el-empty>
    <el-card
      v-else
      v-for="(row, index) in sectionNewsList"
      class="box-card"
      shadow="hover"
      style="padding: 10px"
    >
